...
def nearest_loc(GC, TROPOMI):
    ...
    # i index
    iGC = np.abs(
        GC.lon.values.reshape((-1, 1)) - TROPOMI["longitude"].values.reshape((1, -1))
    )
    iGC = iGC.argmin(axis=0)

    # j index
    jGC = np.abs(
        GC.lat.values.reshape((-1, 1)) - TROPOMI["latitude"].values.reshape((1, -1))
    )
...
def filter_tropomi(data, date, lon_min, lon_max, lat_min, lat_max):
    ...
    data = data.where(
        (data["longitude_center"] >= lon_min) & (data["longitude_center"] <= lon_max),
        drop=True,
    )
    data = data.where(
        (data["latitude_center"] >= lat_min) & (data["latitude_center"] <= lat_max),
        drop=True,
    )
    ...
    data = data.where(
        (
            (data["time"][:, 0] == int(date[:4]))
            & (data["time"][:, 1] == int(date[4:6]))
            & (data["time"][:, 2] == int(date[6:]))
        ),
        drop=True,
    )
...
def process_tropomi(data, date):
    ...
    dates = xr.DataArray(pd.to_datetime(dates), dims=["nobs"]).reset_index(
        "nobs", drop=True
    )
    ...
    pressures = surface_pressure - np.arange(z + 1).reshape((1, -1)) * pressure_interval
    pressures = xr.DataArray(pressures, dims=["nobs", "ilayer"])
    pressures_mid = (
        surface_pressure - (np.arange(z).reshape((1, -1)) + 0.5) * pressure_interval
    )
...
def get_diagnostic(data_dir, diag_name, date):
    ...
    filename = os.path.join(
        data_dir, "GEOSChem." + diag_name + "." + short_date + "_0000z.nc4"
    )
...
def read_GC(ch4_data_dir, ch4_halfstep_data_dir, pedge_data_dir, date):
    ...
    pres = get_diagnostic(pedge_data_dir, "LevelEdgeDiags", date)[["Met_PEDGE"]]
    pres = gc.subset_data_latlon(pres, *s.lats, *s.lons)
    data = xr.merge([data, pres])
    pres.close()

    # Rename variables
    data = data.rename({"SpeciesConc_CH4": "CH4", "Met_PEDGE": "PEDGE"})
...
def correct_GC(ch4_data, ch4_halfstep_data_dir, date, scale_factor=5):
    ...
    vp = (
        xr.open_dataset(
            f"{ch4_halfstep_data_dir}/VerticalProfiles/mean_profile_{(date[:6])}.nc"
        )["SpeciesConc_CH4"]
        * 1e9
    )
    ...
    # the profile
    diff = np.abs(
        xr.ufuncs.log10(ch4_data) / np.log10(scale_factor)
    ...
    if (diff >= 1).any():
        ...
        new = (
            get_diagnostic(f"{ch4_halfstep_data_dir}/OutputDir", "SpeciesConc", date)[
                "SpeciesConc_CH4"
            ]
            * 1e9
        )
...
for index in range(len(raw_files)):
    ...
    start = (int(start_date[:4]) == s.year) and (int(start_date[4:6]) in MONTHS)

    # Check if the end date is in range
    end = (int(end_date[:4]) == s.year) and (int(end_date[4:6]) in MONTHS)
...
for date, filenames in Sat_files.items():
    # print('=========== %s ===========' % date)
    preprocess = lambda d: filter_tropomi(
        d, date, s.lon_min, s.lon_max, s.lat_min, s.lat_max
    )
    TROPOMI = xr.open_mfdataset(
        filenames,
        concat_dim="nobs",
        combine="nested",
        chunks=10000,
    ...
    TROP_CH4 = (
        1e9
        * (TROPOMI["methane_profile_apriori"] / TROPOMI["dry_air_subcolumns"]).values
    )
    TROP_PW = (
        -np.diff(TROPOMI["pressures"])
        / (TROPOMI["pressures"][:, 0] - TROPOMI["pressures"][:, -1]).values[:, None]
    )
    ...
    GC_on_sat = apply_avker(TROPOMI["column_AK"].values, TROP_CH4, TROP_PW, GC_on_sat)
